...
- **Solo "nota", mai "waypoint", nell'UI.** Una nota È un waypoint GPX (un punto traccia `trkpt` invece no); `wp_type` è solo un attributo della nota, quindi neanche lì si dice waypoint — il nome campo dati `wp_type` resta com'è per compatibilità formato (#458).
