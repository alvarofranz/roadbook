...
| `readBundle` | `readBundle(File)` | `Promise<{roadbook, media[]}>` | The raw document plus the bundled photos (`photos/` + `media.json`); throws `docError` when there is no document |
...
Each media item is a bundled photo, `{ type:'photo', name, lat, lon, blob }`. Geotags come
from `media.json` inside the container; files without a manifest entry get `lat:
null, lon: null`. Voice notes are not container entries: they are `voice` blocks inside
`roadbook.json`.
...
- **Bare `roadbook.json`**: a file without the `PK` magic is read directly as the
  JSON document (`container: 'json'`, no entries, no photos).
- **MIME detection**: `mimeOf(name)` maps file extensions for photo re-upload
  (avif, jpg, png, webp). Unknown → `application/octet-stream`.
...
- **`readRdbk` vs `readBundle`**: the former returns only the roadbook object; the
  latter also extracts bundled photos. Use `readBundle` when importing a user-supplied
