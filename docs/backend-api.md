...
| — `social_auth` | Coda condivisa dei due social login: (1) accede all'account già collegato via `google_sub`/`apple_sub`, (2) lo collega a un account con email **verificata** corrispondente, o (3) crea un account *senza password* (richiede `accept_terms`; se manca risponde `need_terms`). Il web manda `confirm` **subito**: la scelta dell'account nel chooser di Google / nel foglio di Apple è già la decisione dell'utente, quindi si accede in una sola chiamata (#519). Senza `confirm` risponde ancora il **probe** (`email` + `exists`, non tocca nulla): lo usano i binari delle app già installate, che portano il proprio JS e cambiano solo con una release sugli store. | nessuna |
...
- `deleted` (#187) è il **cestino**: `rb_delete` ci sposta il roadbook (invece di cancellarlo),
  ...
  creato lazy, non può accedere, username riservato) col vecchio username prefissato al
  titolo, i `.rdbk` spostati nel suo storage, e arrivano **direttamente nel cestino**: 30
