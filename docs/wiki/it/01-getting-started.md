...
L'app funziona **offline** per registrazione, editing e navigazione. Serve connessione solo per: login, salvataggio sul profilo, upload foto, pagine pubbliche.
...
|------|--------------|---------------|
| **Roadbook Recorder** | Registra una traccia GPS live, la puoi arricchire con waypoint nei punti che vorrai impostare come note; puoi anche correlare foto dell'incrocio per ricordare come dovrà essere disegnato il tulip o avvertenze varie | Durante il sopralluogo / la ricognizione sul campo |
| **Editor** | Creare o modificare un roadbook partendo da una registrazione, da un GPX o da un roadbook in formato openrally; ottimizza la traccia, rivedi le foto del sopralluogo, completa le note ed i tulip disegnandoli; la gestione delle frecce e dei CAP è automatica sulla base della traccia sottostante. Al termine puoi esportarlo in formato RDBK, openrally e PDF se preferisci stamparlo | Dopo la registrazione (o da zero) per preparare il roadbook definitivo |
...
| **Tripmaster** | Computer di bordo GPS senza roadbook: odometro totale/parziale, velocità, heading, cronometro, waypoint counter, registrazione GPX | Ricognizioni libere, prove, uscite senza roadbook prestabilito |
...
| **Foto & note vocali** | Una nota vocale fa parte della sua nota, quindi viaggia sempre dentro il `.rdbk`. Le foto finiscono nel `.rdbk` solo se spunti "Includi le foto nel .rdbk" all'export. Vivono sul server (serve login). Da sloggato restano nel device e vanno nel `.rdbk` locale |
