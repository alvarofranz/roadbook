...
| `tulipShape(rb, i, isEnd, isFirst)` | la forma che l'autore ha dato alla traccia attorno alla nota (#945): `{ entry, exit, turn }`. Nei 50 m su un lato (prima = ingresso, dopo = uscita, fermandosi alla nota vicina) 3 o più punti = strada disegnata apposta: una polilinea nel box 230×162 (ruotata su `bearing_in`, scalata a 73/63 px lungo la strada); meno = `null`, la strada dritta classica. `turn`: l'angolo dell'uscita dritta, dove va la strada nei primi 20 m. Mai sopra la nota né sugli incroci dell'autore; mai memorizzata (vedi [note-canvas.md](note-canvas.md) §3) |
