...
| `tulipShape(rb, i, isEnd)` | la forma reale della strada in uscita (#945): `{ exit }`, una polilinea nel box 230×162 (ruotata su `bearing_in`, scalata a 63 px lungo la strada) o `null` dove la strada è dritta. ~50 m lungo la traccia dopo la nota, fermati alla nota successiva, Douglas-Peucker; una curva solo se il tratto si scosta dalla sua corda più del 12% della lunghezza (min 5 m); un tornante si prova su 35/25 m, poi uscita classica; una nota con incroci tiene l'uscita classica. L'ingresso è sempre dritto. Mai memorizzata: si ricava al render (vedi [note-canvas.md](note-canvas.md) §3) |
