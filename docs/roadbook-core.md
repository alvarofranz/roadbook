...
| `tulipShape(rb, i, isEnd, isFirst)` | la forma reale della traccia attorno alla nota (#945): `{ entry, exit }`, polilinee nel box 230×162 (ruotate su `bearing_in`, scalate a 73/63 px lungo la strada) o `null` dove la strada è dritta. ~80 m lungo la traccia per lato, fermati alla nota vicina, Douglas-Peucker; una curva solo se il tratto si scosta dalla sua corda più del 12% della lunghezza (min 6 m); un tornante si prova su 60/45/30 m, poi strada classica. Mai memorizzata: si ricava al render (vedi [note-canvas.md](note-canvas.md) §3) |
