...
describe('per-note map guidance arrow (#485 · #890)', () => {
    ...
    it('the reader draws the guide on open and follows the live fix', () => {
        expect(reader).toContain('guideTo(i, lastHere)');
        expect(reader).toContain('guideTo(inlineMapIdx, here)');
    });
...
describe('async actions report on their own button (#491)', () => {
    ...
    const cases = [
        ...
        ['public/admin/trash/admin-trash.js', "async function restore(rb, btn)"],
...
describe('a map-shaped field is stored as an object (#523)', () => {
    ...
    it('the server refuses a symbol library written as a list, and every read goes through the one payload reader', () => {
        ...
        expect(php.match(/json_encode\(\$rb\)/g).length).toBe(1); // encoded once, and both save branches store the document as it came
...
describe('a refused media delete says why (#525)', () => {
    ...
    it('every media delete reports its outcome on the button that was pressed', () => {
        for (const call of ["RBApi('ph_delete'"]) {
            let at = -1;
    ...
    it('whoever may edit a roadbook may delete its photos', () => {
        const fn = php.match(/function ph_delete\([\s\S]*?\n\}/)[0];
