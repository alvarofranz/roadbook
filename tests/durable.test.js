...
describe('which keys are durable', () => {
    it('are the session, the signed-in user, the pending runs and every crash checkpoint', () => {
        for (const k of ['rb_token', 'rb_cfg_user', 'rb_pending_runs', 'rb_session', 'rb_session_roadbook', 'rb_recorder_session', 'rb_tripmaster_session', 'rb_trip_gpx']) expect(isDurable(k)).toBe(true);
