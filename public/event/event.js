...
(function () {
    ...
    function render(j) {
        ...
        // bust the 7-day .avif cache after a re-upload (parity with the manage view); hide silently on load failure (e.g. AVIF unsupported)
        if (e.logo) { $('evLogo').src = RBMediaSrc(e.logo) + (e.logo.includes('?') ? '&' : '?') + 'v=' + Date.now(); $('evLogo').hidden = false; $('evLogo').onerror = () => { $('evLogo').hidden = true; }; }
