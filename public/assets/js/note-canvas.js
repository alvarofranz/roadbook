...
window.NoteCanvas = class NoteCanvas {
    ...
    _build() {
        this.el.classList.add('ncv');
        this.el.innerHTML = '';
        this.svg = svg('svg', { viewBox: `0 0 ${this.REF_W} ${this.REF_H}`, class: 'ncv-svg' });
        this.el.appendChild(this.svg);
        const defs = svg('defs', {});
        defs.innerHTML = `<marker id="ncv-arrow" viewBox="0 0 10 10" refX="8" refY="5" markerWidth="5" markerHeight="5" orient="auto-start-reverse"><path d="M0 0 L10 5 L0 10 z" fill="context-stroke"></path></marker>`;
        ...
        this.toolbarEl.classList.add('ncv-toolbar');
        // deseleccionar al tocar el fondo
        this.svg.addEventListener('pointerdown', (e) => { if (e.target === this.svg || e.target.classList.contains('ncv-bg')) this.select(null); });
    ...
    render() {
        [...this.svg.querySelectorAll('.ncv-dyn')].forEach((n) => n.remove());
        if (!this.note) { this.toolbarEl.innerHTML = ''; return; }
        const cx = this.REF_W / 2, cy = this.REF_H / 2;
        this.svg.appendChild(svg('rect', { class: 'ncv-dyn ncv-bg', x: 0, y: 0, width: this.REF_W, height: this.REF_H, fill: 'transparent' }));
        this.svg.appendChild(svg('line', { class: 'ncv-dyn', x1: cx, y1: this.REF_H - 8, x2: cx, y2: 18, stroke: 'rgba(255,255,255,.22)', 'stroke-width': 3, 'marker-end': 'url(#ncv-arrow)' }));
        // bivios
        (this.note.junctions || []).forEach((b, i) => {
            ...
            const ln = svg('line', { class: 'ncv-dyn ncv-junctions', 'data-i': i, x1: px, y1: py, x2: tx, y2: ty, stroke: rt.color, 'stroke-width': b.width || 3, 'stroke-linecap': 'round', 'marker-end': 'url(#ncv-arrow)', 'stroke-dasharray': rt.dashed ? '6 4' : '' });
        ...
        (this.note.icons || []).forEach((ic, i) => {
            ...
            const s = ic.size || 32;
            const g = svg('g', { class: 'ncv-dyn ncv-icon', 'data-i': i, transform: `rotate(${ic.angle || 0} ${cxi} ${cyi})` });
            ...
            if (this.sel && this.sel.type === 'icon' && this.sel.i === i) {
                this.svg.appendChild(svg('rect', { class: 'ncv-dyn', x: cxi - s / 2, y: cyi - s / 2, width: s, height: s, fill: 'none', stroke: '#e8b059', 'stroke-width': 1.2, 'stroke-dasharray': '3 2', transform: `rotate(${ic.angle || 0} ${cxi} ${cyi})`, 'pointer-events': 'none' }));
                // drag the corner to resize (rotation-invariant; the +/- buttons still work too)
                const rh = svg('rect', { class: 'ncv-dyn', x: cxi + s / 2 - 5, y: cyi + s / 2 - 5, width: 10, height: 10, rx: 2, fill: '#5aa9ff', stroke: '#0e1116', 'stroke-width': 1.5 });
    ...
    _handle(vx, vy, onMove) {
        const h = svg('circle', { class: 'ncv-dyn', cx: vx, cy: vy, r: 6, fill: '#e8b059', stroke: '#0e1116', 'stroke-width': 1.5 });
        h.style.cursor = 'move';
    ...
    _toolbar() {
        ...
        if (this.sel.type === 'icon') {
            ...
            t.innerHTML = `<select class="ncv-rt" title="Road type">${RB.ROAD_TYPES.map((r, k) => `<option value="${k}" ${k === b.road_type ? 'selected' : ''}>${['Default', 'Motorway', 'Asphalt', 'Track', 'Off-piste'][k]}</option>`).join('')}</select>`
                + btn('fa-minus', 'th-') + btn('fa-plus', 'th+') + btn('fa-trash-can', 'del', false, true);
            t.querySelector('.ncv-rt').onchange = (e) => { b.road_type = +e.target.value; this._chg(); };
...
window.NoteCanvas.rowCols = function (n, iconSrc, vigInline) {
    ...
    const cap = n.cap != null ? `<div class="capind">CAP ${Math.round(n.cap)}°${n.cap_distance != null ? '<br>km ' + (n.cap_distance / 1000).toFixed(2) : ''}</div>` : '';
    ...
    return `<div class="col-l">
            <div class="dtot">${((n.distance ?? 0) / 1000).toFixed(2)}</div>
            <div class="dpar">${((n.partial_distance ?? 0) / 1000).toFixed(2)}</div>
            <div class="nnum">${n.num}</div>${bearing}
        </div>
        <div class="col-c"${hasVig && !vigInline ? ' data-vig' : ''}>${centre}</div>
        <div class="col-r">
...
function btn(icon, action, active, danger) { return `<button data-a="${action}" class="${active ? 'on' : ''} ${danger ? 'dg' : ''}"><i class="fa-solid ${icon}"></i></button>`; }
