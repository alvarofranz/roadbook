...
(window.RBi18nLangs = window.RBi18nLangs || {}).it = {
            ...
            'Unsaved draft': 'Bozza non salvata', 'Recording in progress': 'Registrazione in corso', 'Tripmaster run': 'Percorso Tripmaster', 'Run in progress': 'Percorso in corso', 'Detection radius': 'Raggio di rilevamento', 'This note only': 'Solo questa nota', 'Inherited': 'Ereditato', 'Roadbook default': 'Predefinito del roadbook', 'Note type default': 'Predefinito del tipo di nota', 'System default': 'Predefinito di sistema', 'Roadbook type': 'Tipo di roadbook', 'Basic (adventure)': 'Base (avventura)', 'Rally (FIA)': 'Rally (FIA)', 'Default detection radius (m)': 'Raggio di rilevamento predefinito (m)', 
...
Object.assign(window.RBi18nLangs.it, {
    ...
    'notes.icons': 'Simboli posizionati — vedi <a href="#icons">simboli</a>.',
    'notes.junctions': 'Vettori di incrocio — vedi <a href="#junctions">incroci</a>.',
    'notes.note_kind': 'Opzionale. Con il valore <code>"comment"</code>, la nota è una <b>nota di commento</b> senza navigazione (per esempio il logo di uno sponsor). Non porta <code>lat</code>/<code>lon</code>/<code>idx</code>/<code>num</code> né geodati, mantiene una posizione fissa nell’elenco ed è mostrata nel Reader, nel PDF e nella vista pubblica, ma mappa, punteggio, convalida GPS ed esportazione GPX/KMZ la saltano. Assente = nota di navigazione normale.',
    'notes.image': 'Opzionale (note di commento). Un’immagine incorporata come URI <code>data:</code>, disegnata nel riquadro della nota.',
    'notes.comment.intro': 'Una <b>nota di commento</b> (<code>note_kind: "comment"</code>) non ha coordinate: porta solo il suo testo e un’<code>image</code> incorporata opzionale:',
    ...
    'conformance.comment': 'Un reader conforme DEVE mostrare le note con <code>note_kind: "comment"</code> nell’elenco del roadbook, ma NON DEVE numerarle, posizionarle sulla mappa, valutarle né esportarle come waypoint GPX/KMZ.',
    'Full minimal example': 'Esempio minimo completo',
    'Open a .rdbk in the Reader': 'Apri un .rdbk nel Reader',
});
