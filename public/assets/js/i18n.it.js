...
(window.RBi18nLangs = window.RBi18nLangs || {}).it = {
            ...
            /* editor consistency check (#339) */ 'Consistency check': 'Controllo di coerenza', 'Have a look before saving — none of this stops the roadbook from working:': 'Dai un occhio prima di salvare — niente di tutto questo impedisce al roadbook di funzionare:', 'Notes with no validation radius of their own': 'Note senza un raggio di validazione proprio', 'No roadbook-wide default radius is set, so those notes fall back to the system default.': 'Non è impostato un raggio predefinito del roadbook, quindi quelle note usano il valore predefinito di sistema.', 'A speed-controlled zone is never lifted — it starts at note': 'Una zona a velocità controllata non viene mai revocata — inizia alla nota', 'A speed limit is lifted where no zone is open': 'Un limite di velocità viene revocato dove non c’è nessuna zona aperta', 'Save anyway': 'Salva comunque', 'Let me fix it': 'Preferisco correggere',
            ...
            'Transcription is only available on desktop browsers.': 'La trascrizione è disponibile solo su browser desktop (PC).',
